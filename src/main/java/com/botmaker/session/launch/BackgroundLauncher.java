...
public final class BackgroundLauncher implements AutoCloseable {
    ...
    /**
     * Hands one outcome to {@code report}, on whatever thread produced it.
     *
     * <p>It used to hop to the JavaFX application thread, which is what a caller that touches a button
     * needs — and is not this class's business to know. This module has no JavaFX at all: it is a library a
     * bot links against as readily as an editor does, and one that dragged in a UI toolkit to deliver a
     * callback would be unusable in every headless caller. <b>The caller hops</b>, which is the same rule
     * the plugin contract states for its own listeners.
     */
